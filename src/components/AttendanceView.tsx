    ...
    <div className="max-w-2xl mx-auto px-3 py-4 font-sans space-y-4">
      ...
      {!isCardModeOpen && (
        <>
          ...
          {/* Student Rows List */}
          <div className="bg-white rounded-2xl border border-slate-200/90 divide-y divide-slate-100 shadow-2xs">
                ...
                return (
                  <div
                    ...
                    className={`p-3 sm:p-3.5 space-y-2.5 transition-colors ${
                      status === 'absent'
                        ? 'bg-rose-50/50 border-r-4 border-rose-500'
                        : status === 'present'
                        ? 'bg-emerald-50/30 hover:bg-emerald-50/50 border-r-4 border-emerald-500'
                        : 'bg-white hover:bg-slate-50/60'
                    }`}
                  >
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                      {/* Name & Badges & Quick Toggles (Uniform + Notes) */}
                      <div className="flex items-center justify-between gap-2 min-w-0">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          {cleanImportedString(student.medicalNotes) && (
                            <span
                              className="text-[10px] font-bold text-amber-800 bg-amber-100 border border-amber-300 px-1.5 py-0.5 rounded shrink-0 cursor-help"
                          ...
                          {cleanImportedString(studentNote) && !isNoteOpen && (
                            <button
                              ...
                              className="bg-purple-50 text-purple-800 border border-purple-200 px-2 py-0.5 rounded-lg text-[10px] font-black flex items-center gap-1 cursor-pointer hover:bg-purple-100 transition-colors shrink-0"
                              title="تعديل الملاحظة"
                            >
                              <StickyNote className="w-3 h-3 text-purple-600" />
                              <span className="max-w-[80px] sm:max-w-[110px] truncate">{cleanImportedString(studentNote)}</span>
                            </button>
                          )}
                        </div>

                        {/* Quick Action Badges (Uniform + Note button on the same line on mobile) */}
                          ...
                          {attendanceCheckItems.map((item) => {
                            ...
                            return (
                              <button
                                ...
                                className={`px-2 py-1 rounded-lg text-[11px] font-black border transition-all cursor-pointer select-none flex items-center gap-1 active:scale-95 ${
                                  isChecked
                      ...
                      {/* Attendance Status Action Buttons (Guaranteed 100% visible, wide, prominent) */}
                      <div className="flex items-center gap-2 w-full sm:w-auto justify-start sm:justify-end">
                        {/* Desktop-only Checkbox items & Note toggle */}
                        ...
                        {/* Direct Attendance Action Bar - NEVER CLIPPED */}
                        <div className="flex items-center gap-1.5 bg-slate-100 p-1 rounded-2xl border border-slate-200 w-full sm:w-auto overflow-x-auto scrollbar-none shrink-0">
                          {/* Main Button: حاضر (Fully Prominent & Uncovered) */}
                          <button
                            type="button"
                            onClick={() => setStatus(student.id, 'present')}
                            className={`flex-1 sm:flex-initial min-w-[76px] sm:min-w-[88px] px-3.5 py-2.5 rounded-xl text-xs sm:text-sm font-black transition-all cursor-pointer active:scale-95 flex items-center justify-center gap-1.5 shrink-0 ${
                              status === 'present'
                          ...
                          {/* Separator */}
                          <div className="w-px h-7 bg-slate-300 mx-0.5 shrink-0" />
                          ...
                          <div className="flex items-center gap-1 shrink-0">
                            <button
                              ...
                              className={`px-2.5 py-2 rounded-xl text-xs font-black transition-all cursor-pointer whitespace-nowrap flex items-center gap-1 shrink-0 ${
                                status === 'late'
